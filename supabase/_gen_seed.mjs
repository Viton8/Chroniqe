...
const painScale = [
  { value: '0', label: '0 — не болит', description: 'Боли нет.', color: '#6e6578' },
  { value: '1', label: '1 — едва заметно', description: 'Ощущается, только если специально прислушаться.', color: '#0f766e' },
  { value: '2', label: '2 — слабая', description: 'Лёгкая боль, на неё можно не обращать внимания.', color: '#0f766e' },
  { value: '3', label: '3 — заметная', description: 'Боль есть, но почти не мешает делам.', color: '#0f766e' },
  { value: '4', label: '4 — умеренная', description: 'Отвлекает, но работу ещё можно продолжать.', color: '#b45309' },
  { value: '5', label: '5 — мешает', description: 'Трудно игнорировать, концентрация падает.', color: '#b45309' },
  { value: '6', label: '6 — сильная', description: 'Мешает думать и работать, хочется остановиться.', color: '#b45309' },
  { value: '7', label: '7 — очень сильная', description: 'Сложно выполнять обычные действия.', color: '#c2410c' },
  { value: '8', label: '8 — тяжёлая', description: 'Почти ни о чём другом не получается думать.', color: '#c2410c' },
  { value: '9', label: '9 — невыносимая', description: 'Почти невозможно что-либо делать.', color: '#be123c' },
  { value: '10', label: '10 — максимум', description: 'Худшая боль, какую можно представить.', color: '#9f1239' },
]
const amountScale = [
...
schemas.headaches = {
  fields: [
    f('title', 'Эпизод', 'text', { required: true, config: { maxLength: 160 } }),
    f('kind', 'Тип', 'select', {
      config: {
        options: [
    ...
    f('intensity', 'Интенсивность', 'sublist', {
      description: 'Каждая строка — момент или промежуток. В таблице список свёрнут до пика и границ приступа.',
      config: {
        sublistSummary: { mode: 'peak_span', valueFieldId: 'level', startFieldId: 'at', endFieldId: 'until' },
        subfields: [
          { id: 'at', key: 'at', name: 'С', type: 'datetime', required: true },
          { id: 'until', key: 'until', name: 'До', type: 'datetime' },
          { id: 'level', key: 'level', name: 'Уровень', type: 'select', required: true, config: { options: painScale } },
          { id: 'note', key: 'note', name: 'Комментарий', type: 'text', config: { maxLength: 200 } },
        ],
      },
    }),
    ...
    f('meds', 'Лекарства', 'sublist', {
      config: {
        subfields: [
          { id: 'taken_at', key: 'taken_at', name: 'Принял', type: 'datetime', required: true },
          { id: 'name', key: 'name', name: 'Препарат', type: 'text', required: true, config: { maxLength: 80 } },
          { id: 'dose', key: 'dose', name: 'Доза', type: 'text', config: { maxLength: 40 } },
          { id: 'felt_at', key: 'felt_at', name: 'Эффект ощутил', type: 'datetime' },
          {
            id: 'effect',
            key: 'effect',
            name: 'Эффект',
          ...
          { id: 'note', key: 'note', name: 'Заметка', type: 'text', config: { maxLength: 160 } },
        ],
      },
    }),
    ...
    f('bp_readings', 'Давление', 'relation', {
      config: { relatedListId: L.bp, allowMultiple: true, relationDisplay: 'title' },
    }),
...
schemas.bp = {
  fields: [
    f('title', 'Запись', 'text', { required: true, config: { maxLength: 160 } }),
    f('measured_at', 'Дата и время', 'datetime', { required: true }),
    f('systolic', 'Систолическое', 'integer', { required: true, config: { min: 70, max: 260 } }),
    f('diastolic', 'Диастолическое', 'integer', { required: true, config: { min: 40, max: 160 } }),
    f('pulse', 'Пульс', 'integer', { config: { min: 30, max: 220 } }),
    f('irregular', 'Неровный пульс', 'boolean'),
    f('photo', 'Фото измерения', 'image', { config: { maxSizeMb: 2 } }),
    ...
    f('mins_exercise', 'Минут после нагрузки', 'integer', { config: { min: 0, max: 300 } }),
    f('mins_meal', 'Минут после еды', 'integer', { config: { min: 0, max: 300 } }),
    f('caffeine', 'Кофеин', 'select', { config: { defaultValue: 'none', options: amountScale } }),
    f('nicotine', 'Никотин', 'select', { config: { defaultValue: 'none', options: amountScale } }),
    ...
    f('wellbeing', 'Самочувствие', 'rating', { config: { ratingMax: 10, min: 1, max: 10 } }),
    ...
    f('headache', 'Приступ', 'relation', {
      config: { relatedListId: L.headaches, allowMultiple: true, relationDisplay: 'title' },
    }),
...
const lists = [
  ...
  },
  {
    ...
    description: 'Приступы со шкалой боли во времени: пик в свёрнутой ячейке, симптомы, лекарства и связь с давлением.',
    icon: '🤕',
    template: 'headache_diary',
    schema: schemas.headaches,
    view: views('v-table', [
      { id: 'v-table', name: 'Таблица', kind: 'table', density: 'comfortable', dateFieldId: 'started_at', titleFieldId: 'title' },
      { id: 'v-time', name: 'Лента', kind: 'timeline', dateFieldId: 'started_at', titleFieldId: 'title' },
      { id: 'v-cal', name: 'Календарь', kind: 'calendar', dateFieldId: 'started_at', titleFieldId: 'title' },
      { id: 'v-board', name: 'По типу', kind: 'board', groupFieldId: 'kind', titleFieldId: 'title' },
    ], { mode: 'table', dateFieldId: 'started_at', groupFieldId: 'kind' }),
    settings: { showcase: { featured: true, topic: 'charts' } },
    visibility: 'public',
    edit: 'owner',
  },
  {
    ...
    owner: LENA,
    title: 'Давление',
    description: 'Измерения с пульсом, фото, кофеином, сном и привязкой к приступу боли — графики систолы и пульса.',
    icon: '❤️',
    template: 'blood_pressure',
    schema: schemas.bp,
    view: views('v-table', [
      { id: 'v-table', name: 'Таблица', kind: 'table', density: 'comfortable', dateFieldId: 'measured_at', titleFieldId: 'title' },
      { id: 'v-time', name: 'Лента', kind: 'timeline', dateFieldId: 'measured_at', titleFieldId: 'title' },
      { id: 'v-cal', name: 'Календарь', kind: 'calendar', dateFieldId: 'measured_at', titleFieldId: 'title' },
      { id: 'v-board', name: 'Оценка', kind: 'board', groupFieldId: 'category', titleFieldId: 'title' },
    ], { mode: 'table', dateFieldId: 'measured_at', groupFieldId: 'category' }),
    settings: { showcase: { featured: true, topic: 'charts' } },
    visibility: 'public',
    edit: 'owner',
  },
...
const items = [
  ...
  rowItem(item(801), L.headaches, 1, {
    title: 'Утренняя мигрень',
    kind: 'migraine',
    started_at: '2026-09-08T06:40',
    ended_at: '2026-09-08T14:50',
    ongoing: false,
    intensity: [
      { at: '2026-09-08T06:40', until: '2026-09-08T08:20', level: '5', note: 'проснулась с болью' },
      { at: '2026-09-08T08:20', until: '2026-09-08T10:00', level: '8', note: 'накрыло после кофе' },
      { at: '2026-09-08T10:00', until: '2026-09-08T14:50', level: '4', note: 'после таблетки отпустило' },
    ],
    laterality: 'left',
    location: ['left_temple', 'left_eye', 'forehead'],
    character: ['throbbing', 'exploding'],
    symptoms: ['visual', 'photophobia', 'phonophobia', 'nausea'],
    started_asleep: true,
    ended_asleep: false,
    impact: 'stopped',
    meds: [
      { taken_at: '2026-09-08T08:40', name: 'Ибупрофен', dose: '400 мг', felt_at: '2026-09-08T09:15', effect: 'partial', note: 'тошнота чуть слабее' },
      { taken_at: '2026-09-08T10:05', name: 'Суматриптан', dose: '50 мг', felt_at: '2026-09-08T10:40', effect: 'good' },
    ],
    activity: [
      { at: '2026-09-08T07:10', kind: 'walk', intensity: 'light', effect: 'worse_after', note: 'до кухни — ок, потом хуже' },
    ],
    triggers: ['sleep_loss', 'weather', 'caffeine'],
    helped: ['dark', 'painkiller', 'rest'],
    bp_readings: [{ id: item(821), title: 'Утро, высокое' }, { id: item(822), title: 'После триптана' }],
    notes: 'Аура — зигзаги слева минут на 20. Давление утром было высоким.',
  }, LENA),
  rowItem(item(802), L.headaches, 2, {
    title: 'После мониторов',
    kind: 'tension',
    started_at: '2026-09-05T15:30',
    ended_at: '2026-09-05T19:10',
    ongoing: false,
    intensity: [
      { at: '2026-09-05T15:30', until: '2026-09-05T17:00', level: '3' },
      { at: '2026-09-05T17:00', until: '2026-09-05T19:10', level: '4', note: 'к вечеру давит обручем' },
    ],
    laterality: 'both',
    location: ['forehead', 'crown', 'neck'],
    character: ['pressing', 'tight'],
    symptoms: ['neck'],
    started_asleep: false,
    ended_asleep: false,
    impact: 'slowed',
    meds: [],
    activity: [
      { at: '2026-09-05T18:20', kind: 'yoga', intensity: 'light', effect: 'better_after', note: 'растяжка шеи' },
    ],
    triggers: ['screens', 'neck'],
    helped: ['massage', 'walk', 'air'],
    notes: 'День за ноутбуком без перерывов.',
  }, LENA),
  rowItem(item(803), L.headaches, 3, {
    title: 'Погода и давление',
    kind: 'migraine',
    started_at: '2026-09-02T11:00',
    ended_at: '2026-09-02T18:30',
    ongoing: false,
    intensity: [
      { at: '2026-09-02T11:00', until: '2026-09-02T13:30', level: '6' },
      { at: '2026-09-02T13:30', until: '2026-09-02T16:00', level: '7', note: 'гром на улице' },
      { at: '2026-09-02T16:00', until: '2026-09-02T18:30', level: '3' },
    ],
    laterality: 'right',
    location: ['right_temple', 'occiput'],
    character: ['throbbing', 'dull'],
    symptoms: ['phonophobia', 'nausea', 'dizziness'],
    started_asleep: false,
    ended_asleep: false,
    impact: 'slowed',
    meds: [
      { taken_at: '2026-09-02T13:40', name: 'Цитрамон', dose: '1 табл.', felt_at: '2026-09-02T14:20', effect: 'partial' },
    ],
    activity: [],
    triggers: ['weather', 'stress'],
    helped: ['dark', 'water', 'painkiller'],
    bp_readings: [{ id: item(828), title: 'Стрессовый день' }],
    notes: 'Перед грозой. Давление 154/98.',
  }, LENA),
  rowItem(item(804), L.headaches, 4, {
    title: 'Короткая колющая',
    kind: 'other',
    started_at: '2026-08-28T12:15',
    ended_at: '2026-08-28T12:40',
    ongoing: false,
    intensity: [
      { at: '2026-08-28T12:15', until: '2026-08-28T12:40', level: '3', note: 'несколько вспышек' },
    ],
    laterality: 'right',
    ...
    triggers: [],
    helped: ['time'],
    notes: 'Прошло само, лекарств не брала.',
  }, LENA),
  rowItem(item(805), L.headaches, 5, {
    title: 'Цикл, тяжёлая ночь',
    kind: 'migraine',
    started_at: '2026-08-24T23:50',
    ended_at: '2026-08-25T11:20',
    ongoing: false,
    intensity: [
      { at: '2026-08-24T23:50', until: '2026-08-25T03:00', level: '7', note: 'не могла уснуть' },
      { at: '2026-08-25T03:00', until: '2026-08-25T07:00', level: '9' },
      { at: '2026-08-25T07:00', until: '2026-08-25T11:20', level: '5' },
    ],
    laterality: 'left',
    location: ['left_temple', 'whole', 'neck'],
    character: ['throbbing', 'exploding', 'burning'],
    symptoms: ['visual', 'photophobia', 'phonophobia', 'nausea', 'vomiting', 'yawning'],
    started_asleep: false,
    ended_asleep: true,
    impact: 'stopped',
    meds: [
      { taken_at: '2026-08-25T00:20', name: 'Парацетамол', dose: '500 мг', felt_at: '2026-08-25T01:00', effect: 'none' },
      { taken_at: '2026-08-25T03:10', name: 'Суматриптан', dose: '50 мг', felt_at: '2026-08-25T03:50', effect: 'good' },
    ],
    activity: [],
    triggers: ['hormones', 'sleep_loss'],
    helped: ['painkiller', 'sleep', 'dark', 'cold'],
    notes: 'Заснула под утро, проснулась уже легче.',
  }, LENA),
  rowItem(item(806), L.headaches, 6, {
    title: 'После зала',
    kind: 'mixed',
    started_at: '2026-08-30T19:40',
    ended_at: '2026-08-30T22:15',
    ongoing: false,
    intensity: [
      { at: '2026-08-30T19:40', until: '2026-08-30T20:30', level: '4', note: 'ещё в зале' },
      { at: '2026-08-30T20:30', until: '2026-08-30T22:15', level: '6', note: 'дома усилилась' },
    ],
    laterality: 'both',
    location: ['occiput', 'neck', 'crown'],
    character: ['pressing', 'dull'],
    symptoms: ['neck', 'dizziness'],
    started_asleep: false,
    ended_asleep: false,
    impact: 'slowed',
    meds: [
      { taken_at: '2026-08-30T20:45', name: 'Ибупрофен', dose: '400 мг', felt_at: '2026-08-30T21:20', effect: 'partial' },
    ],
    activity: [
      { at: '2026-08-30T18:30', kind: 'gym', intensity: 'vigorous', effect: 'better_during', note: 'во время сета легче, после — хуже' },
      { at: '2026-08-30T21:00', kind: 'walk', intensity: 'light', effect: 'none' },
    ],
    triggers: ['exercise', 'dehydration'],
    helped: ['water', 'painkiller'],
    bp_readings: [{ id: item(823), title: 'После пробежки' }],
    notes: 'Мало пила на тренировке.',
  }, LENA),
  rowItem(item(807), L.headaches, 7, {
    title: 'Вокруг глаза',
    kind: 'cluster',
    started_at: '2026-09-01T02:10',
    ended_at: '2026-09-01T03:05',
    ongoing: false,
    intensity: [
      { at: '2026-09-01T02:10', until: '2026-09-01T02:40', level: '8' },
      { at: '2026-09-01T02:40', until: '2026-09-01T03:05', level: '5' },
    ],
    laterality: 'right',
    location: ['right_eye', 'right_temple'],
    character: ['stabbing', 'burning'],
    symptoms: ['tearing', 'nasal', 'photophobia'],
    started_asleep: true,
    ended_asleep: false,
    impact: 'stopped',
    meds: [],
    ...
    triggers: [],
    helped: ['air', 'time'],
    notes: 'Короткий ночной приступ, слезился правый глаз.',
  }, LENA),
  rowItem(item(808), L.headaches, 8, {
    title: 'Фон с утра',
    kind: 'tension',
    started_at: '2026-09-08T09:00',
    ended_at: null,
    ongoing: true,
    intensity: [
      { at: '2026-09-08T09:00', level: '2', note: 'ещё держится' },
    ],
    laterality: 'both',
    location: ['forehead'],
    ...
    triggers: ['screens'],
    helped: [],
    notes: 'После утренней мигрени остался лёгкий фон. Конец ещё не ставила.',
  }, LENA),
  rowItem(item(809), L.headaches, 9, {
    title: 'После вина',
    kind: 'migraine',
    started_at: '2026-08-16T10:20',
    ended_at: '2026-08-16T16:00',
    ongoing: false,
    intensity: [
      { at: '2026-08-16T10:20', until: '2026-08-16T12:00', level: '6' },
      { at: '2026-08-16T12:00', until: '2026-08-16T16:00', level: '4' },
    ],
    laterality: 'shifting',
    location: ['forehead', 'left_temple', 'right_temple'],
    character: ['throbbing', 'pressing'],
    symptoms: ['nausea', 'osmophobia', 'photophobia'],
    started_asleep: false,
    ended_asleep: false,
    impact: 'slowed',
    meds: [
      { taken_at: '2026-08-16T10:40', name: 'Аспирин', dose: '500 мг', felt_at: '2026-08-16T11:30', effect: 'partial' },
    ],
    activity: [
      { at: '2026-08-16T13:00', kind: 'walk', intensity: 'light', effect: 'better_during', note: 'на воздухе легче' },
    ],
    triggers: ['alcohol', 'sleep_loss', 'dehydration'],
    helped: ['water', 'sleep', 'air', 'caffeine'],
    notes: 'Ужин с красным вином накануне.',
  }, LENA),
  rowItem(item(810), L.headaches, 10, {
    title: 'Пропуск обеда',
    kind: 'tension',
    started_at: '2026-09-04T14:00',
    ended_at: '2026-09-04T17:45',
    ongoing: false,
    intensity: [
      { at: '2026-09-04T14:00', until: '2026-09-04T15:30', level: '4' },
      { at: '2026-09-04T15:30', until: '2026-09-04T17:45', level: '5', note: 'после совещания' },
    ],
    laterality: 'both',
    location: ['forehead', 'neck', 'occiput'],
    character: ['tight', 'pressing'],
    symptoms: ['neck', 'tinnitus'],
    started_asleep: false,
    ended_asleep: false,
    impact: 'slowed',
    meds: [],
    activity: [
      { at: '2026-09-04T16:10', kind: 'chores', intensity: 'light', effect: 'worse_during' },
    ],
    triggers: ['skipped_meal', 'stress', 'neck'],
    helped: ['water', 'massage', 'rest'],
    notes: 'Поела только в 18:00 — после еды отпустило.',
  }, LENA),

  rowItem(item(821), L.bp, 1, {
    title: 'Утро, высокое',
    measured_at: '2026-09-08T08:15',
    systolic: 148, diastolic: 92, pulse: 78, irregular: false,
    photo: img('bp-morning'),
    comment: 'До таблетки, сидя на кухне',
    position: 'sitting', arm: 'left',
    mins_exercise: null, mins_meal: 40,
    caffeine: 'little', nicotine: 'none',
    sleep_hours: 5.5, sleep_quality: 'poor',
    stress: 'high', wellbeing: 4,
    headache_now: 'severe',
    headache: [{ id: item(801), title: 'Утренняя мигрень' }],
    category: 'high',
    notes: 'Плохо спала, мигрень уже на пике.',
  }, LENA),
  rowItem(item(822), L.bp, 2, {
    title: 'После триптана',
    measured_at: '2026-09-08T11:00',
    systolic: 128, diastolic: 82, pulse: 68, irregular: false,
    photo: img('bp-afterpill'),
    comment: 'Боль спала до 4',
    position: 'sitting', arm: 'left',
    caffeine: 'none', nicotine: 'none',
    sleep_hours: 5.5, sleep_quality: 'poor',
    stress: 'moderate', wellbeing: 6,
    headache_now: 'mild',
    headache: [{ id: item(801), title: 'Утренняя мигрень' }],
    category: 'elevated',
    notes: 'Через час после суматриптана.',
  }, LENA),
  rowItem(item(823), L.bp, 3, {
    title: 'После пробежки',
    measured_at: '2026-08-30T19:10',
    systolic: 138, diastolic: 84, pulse: 96, irregular: false,
    photo: img('bp-run'),
    comment: 'Ещё не остыла',
    position: 'sitting', arm: 'right',
    mins_exercise: 5,
    caffeine: 'none', nicotine: 'none',
    sleep_hours: 7, sleep_quality: 'good',
    stress: 'none', wellbeing: 7,
    headache_now: 'mild',
    headache: [{ id: item(806), title: 'После зала' }],
    category: 'elevated',
  }, LENA),
  rowItem(item(824), L.bp, 4, {
    title: 'Лёжа вечером',
    measured_at: '2026-09-07T22:40',
    systolic: 118, diastolic: 76, pulse: 58, irregular: false,
    position: 'lying', arm: 'left',
    caffeine: 'none', nicotine: 'none',
    sleep_hours: 8, sleep_quality: 'good',
    stress: 'none', wellbeing: 8,
    headache_now: 'none',
    category: 'normal',
    notes: 'Спокойный вечер, без кофе после обеда.',
  }, LENA),
  rowItem(item(825), L.bp, 5, {
    title: 'Много кофе',
    measured_at: '2026-09-03T10:05',
    systolic: 142, diastolic: 88, pulse: 82, irregular: false,
    photo: img('bp-coffee'),
    comment: 'Третья чашка',
    position: 'sitting', arm: 'left',
    mins_meal: 20,
    caffeine: 'a_lot', nicotine: 'none',
    sleep_hours: 6, sleep_quality: 'fair',
    stress: 'mild', wellbeing: 6,
    headache_now: 'none',
    category: 'high',
  }, LENA),
  rowItem(item(826), L.bp, 6, {
    title: 'Стоя в поликлинике',
    measured_at: '2026-09-06T09:30',
    systolic: 132, diastolic: 90, pulse: 88, irregular: false,
    comment: 'Очередь, волновалась',
    position: 'standing', arm: 'right',
    caffeine: 'little', nicotine: 'none',
    sleep_hours: 7, sleep_quality: 'fair',
    stress: 'moderate', wellbeing: 5,
    headache_now: 'none',
    category: 'elevated',
    notes: 'На приёме перемерили сидя — было 124/80.',
  }, LENA),
  rowItem(item(827), L.bp, 7, {
    title: 'После обеда',
    measured_at: '2026-09-04T15:10',
    systolic: 126, diastolic: 82, pulse: 72, irregular: false,
    position: 'sitting', arm: 'left',
    mins_meal: 25,
    caffeine: 'little', nicotine: 'none',
    sleep_hours: 6.5, sleep_quality: 'fair',
    stress: 'moderate', wellbeing: 6,
    headache_now: 'moderate',
    headache: [{ id: item(810), title: 'Пропуск обеда' }],
    category: 'elevated',
    notes: 'Обед был поздний, голова уже ныла.',
  }, LENA),
  rowItem(item(828), L.bp, 8, {
    title: 'Стрессовый день',
    measured_at: '2026-09-02T13:20',
    systolic: 154, diastolic: 98, pulse: 90, irregular: true,
    photo: img('bp-stress'),
    comment: 'Перед созвоном',
    position: 'sitting', arm: 'left',
    caffeine: 'moderate', nicotine: 'little',
    sleep_hours: 5, sleep_quality: 'poor',
    stress: 'high', wellbeing: 3,
    headache_now: 'moderate',
    headache: [{ id: item(803), title: 'Погода и давление' }],
    category: 'high',
    notes: 'Редкий случай — сигарета с коллегой. Пульс неровный.',
  }, LENA),
  rowItem(item(829), L.bp, 9, {
    title: 'Ночное',
    measured_at: '2026-09-01T02:35',
    systolic: 122, diastolic: 78, pulse: 64, irregular: false,
    position: 'sitting', arm: 'left',
    caffeine: 'none', nicotine: 'none',
    sleep_hours: 3, sleep_quality: 'poor',
    stress: 'mild', wellbeing: 4,
    headache_now: 'severe',
    headache: [{ id: item(807), title: 'Вокруг глаза' }],
    category: 'normal',
    notes: 'Разбудила боль вокруг глаза, давление при этом обычное.',
  }, LENA),
  rowItem(item(830), L.bp, 10, {
    title: 'Выходные',
    measured_at: '2026-08-23T11:00',
    systolic: 116, diastolic: 74, pulse: 62, irregular: false,
    photo: img('bp-weekend'),
    position: 'sitting', arm: 'left',
    mins_exercise: 90,
    caffeine: 'little', nicotine: 'none',
    sleep_hours: 8.5, sleep_quality: 'excellent',
    stress: 'none', wellbeing: 9,
    headache_now: 'none',
    category: 'normal',
    notes: 'Прогулка утром, потом измерение.',
  }, LENA),
  rowItem(item(831), L.bp, 11, {
    title: 'Гипотония после жары',
    measured_at: '2026-08-19T16:45',
    systolic: 98, diastolic: 62, pulse: 54, irregular: false,
    comment: 'Головокружение в метро',
    position: 'sitting', arm: 'left',
    caffeine: 'none', nicotine: 'none',
    sleep_hours: 7, sleep_quality: 'good',
    stress: 'none', wellbeing: 5,
    headache_now: 'mild',
    category: 'low',
    notes: 'Жара, мало воды. Голова скорее тяжёлая, не мигрень.',
  }, LENA),
  rowItem(item(832), L.bp, 12, {
    title: 'Контроль у врача',
    measured_at: '2026-08-12T09:00',
    systolic: 124, diastolic: 80, pulse: 70, irregular: false,
    photo: img('bp-clinic'),
    comment: 'Кабинет терапевта, манжета клиники',
    position: 'sitting', arm: 'right',
    mins_meal: 60,
    caffeine: 'none', nicotine: 'none',
    sleep_hours: 7.5, sleep_quality: 'good',
    stress: 'mild', wellbeing: 7,
    headache_now: 'none',
    category: 'normal',
  }, LENA),
]
...
const comments = [
  ...
  [note(17), item(801), ANNA, 'Похоже на мои утренние — держи дневник рядом с тонометром.', 'rose'],
  [note(18), item(805), KIRILL, 'Ночной пик 9 — это уже к врачу, не только дневник.', 'amber'],
  [note(19), item(821), LENA, 'Повторила через 2 минуты — 146/90. Беру первое.', 'slate'],
  [note(20), item(828), ANNA, 'Стресс плюс никотин — логично, что скакнуло.', 'violet'],
]
...
const charts = [
  ...
  [chart(33), L.headaches, 'Приступы по дням', 'timeline', { aggregation: 'count', dateFieldId: 'started_at' }],
  [chart(34), L.headaches, 'Пик боли во времени', 'stem', { aggregation: 'avg', dateFieldId: 'started_at', valueFieldId: 'intensity' }],
  [chart(35), L.headaches, 'Типы приступов', 'pie', { groupFieldId: 'kind' }],
  [chart(36), L.headaches, 'Средний пик', 'kpi', { valueFieldId: 'intensity', aggregation: 'avg' }],
  [chart(37), L.bp, 'Систолическое', 'line', { aggregation: 'avg', dateFieldId: 'measured_at', valueFieldId: 'systolic' }],
  [chart(38), L.bp, 'Пульс', 'line', { aggregation: 'avg', dateFieldId: 'measured_at', valueFieldId: 'pulse' }],
  [chart(39), L.bp, 'Оценки', 'pie', { groupFieldId: 'category' }],
  [chart(40), L.bp, 'Среднее давление', 'kpi', { valueFieldId: 'systolic', aggregation: 'avg' }],
]
