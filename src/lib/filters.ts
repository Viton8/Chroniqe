...
export function titleFieldId(schema: ListSchema): string | undefined {
  return (
    schema.titleFieldId ??
    schema.fields.find((field) => field.type === 'text' || field.type === 'textarea')?.id
  )
}
...
export function countActiveFilters(filters: ListFilters): number {
  let n = Object.values(filters.facets).reduce((sum, values) => sum + (values.length ? 1 : 0), 0)
...
export function itemScore(
  ...
  const all = ratings.filter((row) => row.item_id === item.id && row.field_id === fieldId)
...
export function applyListFilters(
  ...
  return items.filter((item) => {
    ...
    if (filters.ratingMin != null && filters.ratingMin > 0 && filters.ratingFieldId) {
      const score = itemScore(item, filters.ratingFieldId, ratings)
...
function compareField(
  ...
  if (field.type === 'date' || field.type === 'datetime') {
    return (itemDateIso(a.values[field.id]) ?? '').localeCompare(itemDateIso(b.values[field.id]) ?? '')
  }
  return String(a.values[field.id] ?? '').localeCompare(String(b.values[field.id] ?? ''), undefined, {
    numeric: true,
    sensitivity: 'base',
  })
}
